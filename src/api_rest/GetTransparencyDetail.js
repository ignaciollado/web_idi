...
import '../css/formattransparencydetail.css'
...
export default function GetPageByID() {
    ...
    return (
        ...
        <div className="formattransparency">
            {
                !postContent ? <strong>{t('description.loadingNews')}</strong>:
                postContent.map(((postContent) => (
                        <div 
                            className="formattransparency__item" 
                            id={postContent.id} 
                            dangerouslySetInnerHTML={
                                { __html: `
                                
                                <div>
                                   <span className="categories">${ iterateCategorySubnodes (postContent._embedded['wp:term'][0], ) }</span> 
                                </div>  
                                ...
                                <div>
                                    <figure>
                                        ...
                                        <figcaption>${postContent.title.rendered}</figcaption>
                                    </figure>
                                </div>

                                <div className="formattransparency__project_title_tags">
                                    <div>
